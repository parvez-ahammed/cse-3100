...
export default function Home() {
  ...
  useEffect(() => {
    const fetchCharacters = async () => {
      try {
        ...
        setMaxPage(Math.ceil((data.info?.count || 0) / 10));
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-start p-4 pb-8 bg-white text-[#001F3F]">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mt-4 mb-6 w-full max-w-3xl">
        <input
          ...
          className="flex-1 border border-[#001F3F] rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#001F3F]"
        />
        <select
          value={status}
          onChange={(e) => setStatus(e.target.value)}
          className="border border-[#001F3F] rounded-xl px-3 py-2 text-center focus:outline-none focus:ring-2 focus:ring-[#001F3F]"
        >
        ...
        <button
          className="px-4 py-2 min-w-24 rounded-xl bg-[#001F3F] text-white font-medium hover:bg-[#003366] transition"
          onClick={() => {
            setUiPage(1);
            setApiPage(1);
      ...
      <div className="flex justify-center mt-6 space-x-4">
        <button
          ...
          onClick={() => {
            ...
            setUiPage(newPage);
            if ((newPage - 1) % 2 === 0) setApiPage((prev) => prev - 1);
            setSearchTrigger(true);
          }}
        ...
        <span className="text-[#001F3F] my-auto font-semibold">
